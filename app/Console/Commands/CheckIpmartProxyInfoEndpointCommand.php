...
class CheckIpmartProxyInfoEndpointCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ipmart:check-proxy-info {user_id : User ID from users table}';
     ...
     * @var string
     */
    protected $description = 'Check the authenticated IPmart proxy info API endpoint response.';
    ...
    public function handle(Kernel $kernel): int
    {
        ...
        $token = $user->createToken('ipmart-proxy-info-check');

        $request = Request::create('/api/v1/ipmart/proxy-info', 'GET');
        ...
        $trafficBalance = data_get($payload, 'data.traffic_balance');

        if (! is_numeric($trafficBalance)) {
            $this->error('IPmart proxy-info response is missing a numeric traffic_balance value.');
            $this->line(json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        ...
        $ipmartAccount->update([
            'available_traffic' => (int) $trafficBalance,
        ]);

        $this->info('IPmart proxy-info endpoint responded successfully.');
