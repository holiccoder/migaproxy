...
class TestIpmartPayForCustomerUsingBalanceEndpointCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ipmart:test-pay-for-customer-using-balance-endpoint {user_id : User ID from users table} {amount : Capacity amount to order}';
     ...
     * @var string
     */
    protected $description = 'Test the authenticated IPmart pay-for-customer-using-balance API endpoint response.';
    ...
    public function handle(Kernel $kernel): int
    {
        ...
        $token = $user->createToken('ipmart-pay-balance-check');

        $request = Request::create('/api/v1/ipmart/pay-for-customer-using-balance', 'POST', [
            'amount' => $amount,
        ]);
        ...
        $this->info('IPmart pay-for-customer-using-balance endpoint responded successfully.');
